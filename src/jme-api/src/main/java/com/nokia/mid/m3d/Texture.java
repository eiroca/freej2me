...
public class Texture {
  ...
  private final double[] uvm = new double[9]; // UV matrix
  private final double[] sourcem = new double[9]; // source triangle
  private final double[] transm = new double[9]; // transformation matrix
  private final double[] tempm = new double[9]; // scratch
  ...
  public Texture(final int a, final int b, final Image c) {
  ...
  public int map(final int x, final int y) {
    final int px = (int)((transm[0] * x) + (transm[1] * y) + transm[2]);
    final int py = (int)((transm[3] * x) + (transm[4] * y) + transm[5]);
    final int idx = px + (py * width);
    if ((idx > 0) && (idx < imagedata.length)) { return colors[imagedata[idx] & 1]; }
    return 0;
  }

  public void mapto(final int x1, final int y1, final int x2, final int y2, final int x3, final int y3) {
  ...
  public void setUVs(final int u1, final int v1, final int u2, final int v2, final int u3, final int v3) {
  ...
  //invert 3x3 matrix
  private void inverse(final double[] m) {
