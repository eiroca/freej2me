...
public class M3D {
  ...
  private final double[] stack = new double[16];
  private final double[] tempm = new double[16];
  private final double[] tempr = new double[16];
  private final double[] tempt = new double[16];
  private final double[] temps = new double[16];
  private final double[] rotm = new double[16];
  private final double[] projm = new double[16];
  private int width;
  private int height;
  private Texture texture;
  private boolean boundTexture = false;
  private final double[] verts = new double[600];
  private final double[] UVs = new double[600];
  ...
  public void setupBuffers(final int flags, final int displayWidth, final int displayHeight) {
  ...
  public void cullFace(final int a) {
  ...
  public void clear(final int a) {
    //what is a? c.z (Get Ready) passes 0x4100 (16640)
  ...
  public void frustumxi(final int left, final int right, final int top, final int bottom, final int near, final int far) {
    //-3,3, -2,2, 3,1000
    // c.c: bu.frustumxi(-bp << 11, bp << 11, -bo << 11, bo << 11, 196608, 65536000);
    final double r = right / 2048;
    final double l = left / 2048;
    final double t = top / 2048;
    final double b = bottom / 2048;

    final double n = near / 65536;
    final double f = far / 65536;
    projection(projm, r - l, b - t, 90, n, f);
  }

  public void scalexi(final int X, final int Y, final int Z) {
    final double x = (X / 65536.0);
    final double y = (Y / 65536.0);
    final double z = (Z / 65536.0);
  ...
  public void translatexi(final int X, final int Y, final int Z) {
    final double x = (X / 65536.0);
    final double y = (Y / 65536.0);
    final double z = (Z / 65536.0);
    tempt[0] = 1;
  ...
  public void rotatexi(final int Y, final int Z, final int X, final int W) {
    // probably not a quaternion
    Mobile.debug("rotatexi(" + Y + ", " + Z + ", " + X + ", " + W + ");");
    ...
    final double x = (X / 65536.0) * 0.0174533;
    final double y = ((Y / 65536.0) - 10) * 0.0174533;
    final double z = (Z / 65536.0) * 0.0174533;
  ...
  public void vertexPointerub(final int a, final int b, final byte[] vertices) {
  ...
  public void color4ub(final byte r, final byte g, final byte b, final byte a) {
    color = ((0xFF) << 24) | ((r & 0xFF) << 16) | ((g & 0xFF) << 8) | (b & 0xFF);
  }

  public void clearColor4ub(final byte r, final byte g, final byte b, final byte a) {
    clearcolor = ((0xFF) << 24) | ((r & 0xFF) << 16) | ((g & 0xFF) << 8) | (b & 0xFF);
  }

  public void drawElementsub(final int a, final int b, final byte[] faces) {
    gc.setColor(color);
    ...
    final double ox = width / 2;
    final double oy = height / 2;
  ...
  public void drawArrays(final int a, final int b, final int c) {
    // called after clear -- background?
  ...
  public void bindTexture(final int a, final Texture b) {
    texture = b;
    boundTexture = true;
  }

  public void texCoordPointerub(final int a, final int b, final byte[] uvs) {
    for (int i = 0; i < uvs.length; i++) {
      UVs[i] = uvs[i];
    }
  }

  public void enableClientState(final int flags) {
  }

  public void disableClientState(final int flags) {
  }
  ...
  public void blit(final Graphics g, final int x, final int y, final int w, final int h) { // 0, 0, 95, 65
    g.drawImage(platformImage, x, y, Graphics.LEFT | Graphics.TOP);
  }

  private void identity(final double[] m) {
    m[0] = 1;
    m[1] = 0;
    m[2] = 0;
    m[3] = 0;
  ...
  private void projection(final double[] m, final double w, final double h, double fov, final double near, final double far) {
    fov = fov * 0.0174533;
    final double aspect = h / w;
    final double sy = 1 / Math.tan(fov / 2);
    final double sx = sy / aspect;
    final double c = (far + near) / (near - far);
    final double d = -1;
    final double e = (2 * far * near) / (near - far);
    final double f = 0;
  ...
  private void clone(final double[] m1, final double[] m2) {
    for (int i = 0; i < 16; i++) {
      m1[i] = m2[i];
    }
  }

  private void matmul(final double[] m1, final double[] m2) {
  ...
  private void invert(final double[] m) {
    final double a0 = m[0], a1 = m[1], a2 = m[2], a3 = m[3];
    final double a4 = m[4], a5 = m[5], a6 = m[6], a7 = m[7];
    final double a8 = m[8], a9 = m[9], a10 = m[10], a11 = m[11];
    final double a12 = m[12], a13 = m[13], a14 = m[14], a15 = m[15];

    final double b0 = (a0 * a5) - (a1 * a4), b1 = (a0 * a6) - (a2 * a4);
    final double b2 = (a0 * a7) - (a3 * a4), b3 = (a1 * a6) - (a2 * a5);
    final double b4 = (a1 * a7) - (a3 * a5), b5 = (a2 * a7) - (a3 * a6);
    final double b6 = (a8 * a13) - (a9 * a12), b7 = (a8 * a14) - (a10 * a12);
    final double b8 = (a8 * a15) - (a11 * a12), b9 = (a9 * a14) - (a10 * a13);
    final double b10 = (a9 * a15) - (a11 * a13), b11 = (a10 * a15) - (a11 * a14);
  ...
  private void applyMatrix(final double[] m) {
    double x, y, z;
  ...
  private void fillTriangle(final double x1, final double y1, final double z1, final double x2, final double y2, final double z2, final double x3, final double y3, final double z3) {
    double a, b, c, d;
    double depth = 0;
    // find rect, clip to screen
    final int maxX = (int)Math.min(Math.max(x1, Math.max(x2, x3)), width - 1);
    final int maxY = (int)Math.min(Math.max(y1, Math.max(y2, y3)), height - 1);
    final int minX = (int)Math.max(Math.min(x1, Math.min(x2, x3)), 0);
    final int minY = (int)Math.max(Math.min(y1, Math.min(y2, y3)), 0);
